package com.datn.thesocialnetwork.core.ui
...
import com.datn.thesocialnetwork.core.ui.SpannedGridLayoutManager.GridSpanLookup
import com.datn.thesocialnetwork.core.ui.SpannedGridLayoutManager.SpanInfo
                ...
                override fun getSpanInfo(position: Int): SpanInfo {
                    if (type == TYPE_FACEBOOK) {
                        val spanCount = when (adapter?.itemCount ?: childCount) {
                            1 -> 6
                            2 -> return SpanInfo(3, 6)
                            3 -> {
                                when (position) {
                                    0 -> return SpanInfo(4, 6)
                                    else -> return SpanInfo(2, 3)
                                }
                            }
                            4 -> {
                                when (position) {
                                ...
                                when {
                                    position < 2 -> return SpanInfo(3, 4)
                                    else -> 2
                                }
                            }
                        }
                        return SpanInfo(spanCount, spanCount)
                    } else {
                        return SpanInfo(2, 2)
                    }
                }
