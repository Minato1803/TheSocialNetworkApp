    ...
    private fun observeDataLogin(response: Response<UserResponse>?) {
        when (response) {
            ...
            is Response.Success -> {
                ...
                response.data.let {
                    GlobalValue.USER = it
                    GlobalValue.USER_DETAIL = mapToUserModel(GlobalValue.USER!!)
                    mLoginViewModel.setRememberUserId(GlobalValue.USER?.uidUser)
                    val userDetail = ModelMapping.createUserDetail(it!!.userDetail, 0)
                    profileViewModel.updateOnlineStatus(it!!.uidUser, userDetail)
                    sendToMainActivity()
                }
            }
        }
